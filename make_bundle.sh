...
if [ -z "$EXECUTABLE_NAME" ] || [ -z "$BUNDLE_NAME" ] || [ -z "$BUNDLE_ID" ]
then
	echo "usage: $0 executable_name bundle_name bundle_identifier [icon_file_basename]"
	exit 1
fi
...
cp "$EXECUTABLE_NAME" "$EXECUTABLE_NAME.app/Contents/MacOS/$EXECUTABLE_NAME"
